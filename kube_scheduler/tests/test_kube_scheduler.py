...
def test_check_metrics(aggregator, mock_metrics, mock_leader):
    ...
    assert_metric('.pod_preemption_victims.count', value=0.0, tags=[])
    assert_metric('.preemption_attempts.count', value=10.0, tags=[])
    ...
    assert_metric('.schedule_attempts.count', value=15.0, tags=['result:scheduled'])
    assert_metric('.apiserver.audit.requests_rejected.count', value=0.0, tags=[])
    assert_metric('.apiserver.audit.event.count', value=0.0, tags=[])
    assert_metric('.apiserver.storage.data_key_generation_failures.count', value=0.0, tags=[])
    ...
    assert_metric('.apiserver.storage.envelope_transformation_cache_misses.count', value=0.0, tags=[])
    assert_metric('.cache.lookups.count')
